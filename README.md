...
- 如果标准尺寸仍 OOM，可临时改回 `ENABLE_CPU_OFFLOAD=true`。默认 `CPU_OFFLOAD_MODE=model` 会启用 `enable_model_cpu_offload`，按 text encoder / transformer / VAE 等模型组件在 CPU 和 GPU 间按需切换，通常比 sequential 快很多，但省显存幅度较小。显存仍不够时再设 `CPU_OFFLOAD_MODE=sequential`，它会启用 `enable_sequential_cpu_offload` 做更细粒度卸载，峰值显存最低但会明显变慢。同时确认没有其他进程占用显存，不建议再通过 `resolution` 或 `size` 扩大输出尺寸。
